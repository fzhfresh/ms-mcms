...
function initProductSlider() {
    ...
    productDots.forEach((dot, index) => {
        dot.addEventListener("click", () => {
            ...
            productInterval = setInterval(nextBannerSlide, 5000); // 这里应该是 nextProductSlide
        });
    });
    ...
    productSlider.addEventListener("mouseleave", () => {
        productInterval = setInterval(nextProductSlide, 8000); // 这里应该是 nextProductSlide
    });
}
...
Object.entries(dialogBoxes).forEach(([key, dialog]) => {
    if (dialog.btn && dialog.box && dialog.close) {
        dialog.btn.addEventListener('click', () => {
            if (currentOpenDialog && currentOpenDialog !== key) {
                closeAllDialogs();
            }
            if (!isVisible(dialog.box)) {
